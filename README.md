...
>
> **실측 vs 합성 표기 원칙**: 이 문서와 대시보드의 모든 수치는 ⚪ **실측**(공개 데이터에서 직접 수집) 또는 🟣 **합성**(제가 설계한 가상 데이터 분석 결과, 실제 Spigen 내부 데이터 아님)으로 표시합니다. 대시보드에서는 실측을 흰색 실선 배지, 합성을 보라색 점선 배지로 구분하고, 독일(청록)·이탈리아(황색) 시장 색과는 별개로 씁니다. 두 성격을 한 문장에 섞지 않는 것을 전체 작업의 원칙으로 삼았습니다.
